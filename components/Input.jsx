...
function Input() {
    ...
    return (
        <div className={`border-b border-gray-700 p-3 flex space-x-3 scrollbar-hide overflow-y-scroll ${
            loading && "opacity-60"
          }`}>
            <img className="h-11 w-11 rounded-full cursor-pointer" src="" alt="" />
                ...
                <div className={`${selectedFile && "pb-7"} ${input && "space-y-2.5"}`}>
                    ...
                    {selectedFile && (
                        <div className='relative'>
                            ...
                            <img src={selectedFile} alt="Image selected" className='rounded-2xl max-h-80 object-contain' />
                        </div>
                    )}
                </div>
                {!loading && (
                    ...
                    <div className='flex items-center justify-between pt-2.5'>
                        ...
                        <button onClick={sendPost} disabled={!input.trim() && !selectedFile} className="bg-[#1d9bf0] text-white rounded-full px-4 py-1.5 font-bold shadow-md hover:bg-[#1a8cd8] disabled:opacity-50 disabled:cursor-default">
                            Tweet
                        </button>
                    </div>
                )}
